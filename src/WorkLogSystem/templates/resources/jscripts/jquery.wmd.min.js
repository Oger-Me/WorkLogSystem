...
var spacerIndex = 0;function addSpacer(){var spacer=document.createElement("li");switch(spacerIndex){case 0:spacer.className = "wmd-spacer wmd-spacer1";break;case 1:spacer.className = "wmd-spacer wmd-spacer2";break;case 2:spacer.className = "wmd-spacer wmd-spacer3";break;}buttonRow.appendChild(spacer);spacerIndex=spacerIndex+1;return spacer;}
var boldButton=addButton("wmd-bold-button","加粗 <strong> Ctrl+B",command.doBold);var italicButton=addButton("wmd-italic-button","斜体 <em> Ctrl+I",command.doItalic);var spacer1=addSpacer();var linkButton=addButton("wmd-link-button","链接 <a> Ctrl+L",function(chunk,postProcessing,useDefaultText){return command.doLinkOrImage(chunk,postProcessing,false);});var quoteButton=addButton("wmd-quote-button","引用 <blockquote> Ctrl+Q",command.doBlockquote);var codeButton=addButton("wmd-code-button","代码片段 <pre><code> Ctrl+K",command.doCode);var imageButton=addButton("wmd-image-button","图片 <img> Ctrl+G",function(chunk,postProcessing,useDefaultText){return command.doLinkOrImage(chunk,postProcessing,true);});var spacer2=addSpacer();var olistButton=addButton("wmd-olist-button","数字列表 <ol> Ctrl+O",function(chunk,postProcessing,useDefaultText){command.doList(chunk,postProcessing,true,useDefaultText);});var ulistButton=addButton("wmd-ulist-button","普通列表 <ul> Ctrl+U",function(chunk,postProcessing,useDefaultText){command.doList(chunk,postProcessing,false,useDefaultText);});var headingButton=addButton("wmd-heading-button","标题 <h1>/<h2> Ctrl+H",command.doHeading);var hrButton=addButton("wmd-hr-button","分隔线 <hr> Ctrl+R",command.doHorizontalRule);var spacer3=addSpacer();var undoButton=addButton("wmd-undo-button","撤消 - Ctrl+Z");undoButton.execute=function(manager){manager.undo();};var redo_title=null;var redoButton=addButton("wmd-redo-button","重做 - Ctrl+Y");if(/win/.test(nav.platform.toLowerCase())){redoButton.title="重做 - Ctrl+Y";}
else{redoButton.title="重做 - Ctrl+Shift+Z";}
